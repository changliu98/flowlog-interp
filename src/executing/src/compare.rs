...
/// Left-to-right arithmetic over the engine's integers: wrapping on
/// overflow, `None` on a zero divisor.
pub fn arithmetic_ints(init: Val, rest: &[(&ArithmeticOperator, Val)]) -> Option<Val> {
    let mut result = init;
...
/// Record a zero divisor as the evaluation's fault: the row is dropped and
/// the evaluation ends with the diagnostic rather than a silently smaller
/// answer or a crashed worker.
fn division_fault(budget: &Budget, expression: &dyn std::fmt::Display) {
    budget.fault(Diagnostic::evaluation(format!(
...
pub fn arithmetic_row(v: &dyn Array, arithmetic_expr: &ArithmeticArgument) -> Option<Val> {
...
pub fn compare_row(v: &dyn Array, compare_expr: &ComparisonExprArgument, budget: &Budget) -> bool {
    let (Some(left), Some(right)) = (
        arithmetic_row(v, compare_expr.left()),
        arithmetic_row(v, compare_expr.right()),
    ) else {
        division_fault(budget, compare_expr);
        return false;
    };
...
pub fn jn_compare(k: Option<&dyn Array>, v1: Option<&dyn Array>, v2: Option<&dyn Array>, compare_expr: &ComparisonExprArgument, budget: &Budget) -> bool {
    let (Some(left), Some(right)) = (
        jn_arithmetic(k, v1, v2, compare_expr.left()),
        jn_arithmetic(k, v1, v2, compare_expr.right()),
    ) else {
        division_fault(budget, compare_expr);
        return false;
    };
...
pub fn jn_arithmetic(k: Option<&dyn Array>, v1: Option<&dyn Array>, v2: Option<&dyn Array>, arithmetic_expr: &ArithmeticArgument) -> Option<Val> {
